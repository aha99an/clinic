...
def import_csvappo(request):
    ...
    errors_filename = os.path.join(
        dirname, "../csv_files/last_update/appointment-2022-10-20_errors.csv"
    )
    with open(filename, "r", encoding="utf-16") as file, open(
        errors_filename, "w", encoding="utf-16"
    ) as errors_file:
        ...
        if search_date == None:
            ...
            if len(search_date) == 7 or len(search_date) == 6:
                ...
                queryset = queryset.filter(
                    appointmentDate__year=my_search_year,
                    appointmentDate__month=my_search_mounth,
                )
                ...
                queryset = queryset.filter(
                    appointmentDate__year=my_search_year,
                    appointmentDate__month=my_search_mounth,
                    appointmentDate__day=my_search_day,
                )
...
class AppointmentsPatientDetailView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ctx["appointmentsforpatient"] = Appointment.objects.filter(
            patient_id=my__patient.id
        )
